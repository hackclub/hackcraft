...
export async function getAccessToken(type: "hca" | "hackatime") {
  ...
  if (!access_token)
    redirect(
      `https://${type == "hca" ? "auth" : type}.hackclub.com/oauth/authorize?client_id=${process.env[type.toUpperCase() + "_CLIENT_ID"]}&redirect_uri=https%3A%2F%2Flocalhost:3000%2Fapi%2F${type}%2Fcallback&response_type=code&scope=${type == "hca" ? "name+email+slack_id+verification_status+basic_info+address" : "profile+read"}`,
    );
  return access_token;
}
